...
export class AddProduto extends Component {
    ...
    async intialize() {
        ...
        if (id > 0) {
            const response = await fetch('api/Produtoes/' + id);
    ...
    handleSalve(event) {
        ...
        if (this.state.produto.id) {
            const response1 = fetch('api/Produtoes/' + this.state.produto.id, { method: 'PUT', body: data });
            this.props.history.push('/fetch-produto');
        }
        else {
            const response2 = fetch('api/Produtoes/', { method: 'POST', body: data });
            this.props.history.push('/fetch-produto');
        }
    }
